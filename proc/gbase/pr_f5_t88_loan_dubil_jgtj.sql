...
CREATE DEFINER="cqbank_sj"@"%" PROCEDURE "pr_f5_t88_loan_dubil_jgtj"(
 ...
 * whd 20221226 修改所有的汇总表为按系统拆分的子表 如：t03_agmt_subj_rel_h，避免依赖FDS,OPI等系统
 * 贷款借据
 *******************************/
	
	DECLARE ETL_T_TAB_ENG_NAME 		VARCHAR(30)		DEFAULT 't88_loan_dubil_jgtj';
...
-- ETL_STEP_NO = 1 
	SET @SQL_STR = '
DROP   TABLE IF EXISTS pdm.VT_BAL_t88_loan_dubil_jgtj';
...
-- ETL_STEP_NO = 2 
	SET @SQL_STR = '
CREATE  TABLE pdm.VT_BAL_t88_loan_dubil_jgtj LIKE pdm.t88_loan_dubil_jgtj';
...
-- ETL_STEP_NO = 3 
	SET @SQL_STR = '
DROP  TABLE IF EXISTS pdm.AT_BAL_t88_loan_dubil_jgtj';
...
-- ETL_STEP_NO = 4 
	SET @SQL_STR = '
CREATE   TABLE pdm.AT_BAL_t88_loan_dubil_jgtj LIKE pdm.t88_loan_dubil_jgtj';
...
-- ETL_STEP_NO = 5
	SET @SQL_STR = '
DROP  TABLE IF EXISTS pdm.AT_t88_loan_dubil_jgtj';
...
-- ETL_STEP_NO = 6 
	SET @SQL_STR = '
CREATE   TABLE pdm.AT_t88_loan_dubil_jgtj LIKE pdm.t88_loan_dubil_jgtj';
...
-- ETL_STEP_NO = 7	
	 SET @SQL_STR = '
INSERT INTO pdm.VT_BAL_t88_loan_dubil_jgtj(
		Statt_Dt				,-- 	1	统计日期
		Agmt_ID					,-- 	2	协议编号
...
LEFT JOIN pdm.t88_exchg_rate t7 -- 汇率牌价表	-- NCS_IRL_CCY_RATE
...
LEFT JOIN (
	SELECT   a.Agmt_Id, 
		...
		(
			select * from pdm.t03_agmt_subj_rel_ncs where Statt_Dt = ${TX_DATE}
			UNION ALL
			select * from pdm.t03_agmt_subj_rel_sl where Statt_Dt = ${TX_DATE}
			UNION ALL
			select * from pdm.t03_agmt_subj_rel_rmps where Statt_Dt = ${TX_DATE}
		) a
   LEFT JOIN -- pdm.t03_agmt_rel_h b
		(
			select * from pdm.t03_agmt_rel_sl_h where Start_Dt <= ${TX_DATE} and end_dt >= ${TX_DATE}
			UNION ALL
			select * from pdm.t03_agmt_rel_ncs_h where Start_Dt <= ${TX_DATE} and end_dt >= ${TX_DATE}
		) b
   ...
   LEFT JOIN -- pdm.t03_agmt_bal_h c
		(
			select * from pdm.t03_agmt_bal_ncs_h where Start_Dt <= ${TX_DATE} and end_dt >= ${TX_DATE}
			UNION ALL
			select * from pdm.t03_agmt_bal_sl_h where Start_Dt <= ${TX_DATE} and end_dt >= ${TX_DATE}
			UNION ALL
			select * from pdm.t03_agmt_bal_rmps_h where Start_Dt <= ${TX_DATE} and end_dt >= ${TX_DATE}
		) c
	...
	WHERE a.Agmt_Cate_Cd in (\'1101\',\'0102\') -- 贷款账户
		...
		-- AND a.Start_Dt <= ${TX_DATE}  AND a.End_Dt >= ${TX_DATE}
		AND A.STATT_DT = ${TX_DATE} 
...
LEFT JOIN pdm.t03_agmt_bal_NCM_h t13 -- 微粒贷贷款余额
...
-- ETL_STEP_NO = 10
	SET @SQL_STR = '
insert into pdm.AT_BAL_t88_loan_dubil_jgtj(
		Statt_Dt				,-- 	1	统计日期
		Agmt_ID					,-- 	2	协议编号
		Dtl_Subj_ID				,-- 	12	明细科目编号
		Dubil_Bal				,-- 	71	借据余额
...
FROM pdm.VT_BAL_t88_loan_dubil_jgtj t 

 FULL JOIN (
	SELECT * FROM PDM.t88_loan_dubil_jgtj 
...
-- ETL_STEP_NO = 11	
	 SET @SQL_STR = '
INSERT INTO pdm.AT_t88_loan_dubil_jgtj(
		Statt_Dt				,-- 	1	统计日期
		Agmt_ID					,-- 	2	协议编号
		Dubil_Id					,-- 86 借据编号
		Cust_ID					,-- 	3	客户编号
...
select ${TX_DATE} as Statt_Dt					,-- 	1	统计日期
		...
		nvl(t3.Fin_Org_Cd,'''') as Bank_Org_Cd			,-- 	8	金融机构编码
		nvl(t3.Fin_Lics,'''')	as Fin_Lics				,-- 	9	金融许可证号
		...
		'''' as Dtl_Subj_Nm, -- nvl(t4.Subj_Nm,'''') as Dtl_Subj_Nm				,-- 	13	科目名称
		...
		''t03_loan_dubil_h'' as Data_Src_Cd  							 -- 	85	数据来源表名
FROM PDM.AT_BAL_t88_loan_dubil_jgtj AT
...
LEFT JOIN -- PDM.t03_agmt_bal_h T14 
	(
		select * from pdm.t03_agmt_bal_ncm_h where Start_Dt <= ${TX_DATE} and End_Dt >= ${TX_DATE}
		UNION ALL
		select * from pdm.t03_agmt_bal_rmps_h where Start_Dt <= ${TX_DATE} and End_Dt >= ${TX_DATE}
	) T14
	 ON T.Agmt_Id = T14.Agmt_Id
	AND T.Agmt_Cate_Cd = T14.Agmt_Cate_Cd
	-- AND T14.Start_Dt <= ${TX_DATE} AND T14.End_Dt >= ${TX_DATE}
...
LEFT JOIN -- pdm.t04_org t3 
		pdm.t04_crdt_org_h t3	-- 信贷机构历史
	ON t.Ent_Acct_Org_Id = t3.Org_Id 
	-- AND t3.Data_Src_Cd =''NCM'' 
	AND t3.Start_Dt <= ${TX_DATE}  
	AND t3.End_Dt >= ${TX_DATE}
	
/* LEFT JOIN pdm.t10_gl_bas_info_h t4 -- 会计科目基本信息历史 只取了科目名称 ods只有20210714号以后有数据
...
/* t03_loan_dubil_h 表增加了 Crdt_Use_ID字段
...
LEFT JOIN -- pdm.t03_agmt_rate_h t10
	(
	select * from pdm.t03_agmt_rate_ncm_h where start_dt <= ${TX_DATE} and end_dt >= ${TX_DATE}
	UNION ALL
	select * from pdm.t03_agmt_rate_rmps_h where start_dt <= ${TX_DATE} and end_dt >= ${TX_DATE}
	) t10
	ON t8.Agmt_Id = t10.Agmt_Id 	
	-- and t8.Agmt_Cate_Cd = t10.Agmt_Cate_Cd
	AND t10.Agmt_Rate_Typ_Cd = ''01''
	-- AND t10.start_dt <= ${TX_DATE} AND t10.end_dt >= ${TX_DATE}
...
-- ETL_STEP_NO = 14	
	...
	FROM 
	(
		SELECT Agmt_ID FROM pdm.AT_t88_loan_dubil_jgtj
		GROUP BY 1
		HAVING COUNT(*) > 1
	) A ;
	IF PK_COUNT > 0
...
-- ETL_STEP_NO = 15	
	...
	SET @SQL_STR = '
DELETE FROM pdm.t88_loan_dubil_jgtj WHERE Statt_Dt >= ${TX_DATE} ' ;
...
-- ETL_STEP_NO = 16
	...
	SET @SQL_STR = '
INSERT INTO pdm.t88_loan_dubil_jgtj SELECT * FROM pdm.AT_t88_loan_dubil_jgtj where Statt_Dt = ${TX_DATE} ';
...
/*	-- 删除今日临时表
	-- ETL_STEP_NO = 16
	SET @SQL_STR = '
DROP  TABLE pdm.VT_t88_loan_dubil_jgtj';
	...
	-- ETL_STEP_NO = 17
	SET @SQL_STR = '
DROP  TABLE pdm.AT_t88_loan_dubil_jgtj';
