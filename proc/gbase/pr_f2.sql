...
CREATE DEFINER="cq_sjpt"@"%" PROCEDURE "pr_f2"(
...
**********************************/
	...
	DECLARE ETL_S_TAB_ENG_NAME			VARCHAR(200);
    ...
    AND DATA_DATE >= ETL_TX_DATE INTO ETL_RESET_RUN;
 	...
 	IF ETL_RESET_RUN > 0 then
 		-- 删除新增的数据
		SET @DELETE_OD_DATA =  CONCAT('DELETE FROM ',ETL_SCHEMA_NAME, '.', ETL_T_TAB_ENG_NAME ,' WHERE TX_DT >= ${TX_DATE}');
	...
	CALL ETL.PR_EXEC_SQL(@RTC,'',ETL_T_TAB_ENG_NAME,1000004,@DROP_VT_SQL,ETL_TX_DATE);
	IF @RTC <> 0 THEN SET OUT_RES_MSG = '12';LEAVE LABLE;END IF;
	...
	CALL ETL.PR_EXEC_SQL(@RTC,'',ETL_T_TAB_ENG_NAME,1000005,@CREATE_VT_SQL,ETL_TX_DATE);
	IF @RTC <> 0 THEN SET OUT_RES_MSG = '12';LEAVE LABLE;END IF;
	...
	SET @EXEC_MERGE_SQL = CONCAT('INSERT INTO ',ETL_SCHEMA_NAME,'.',ETL_T_TAB_ENG_NAME,' \n','SELECT * FROM ',ETL_SCHEMA_NAME,'.',ETL_T_TAB_ENG_NAME_VT);
	
	CALL ETL.PR_EXEC_SQL(@RTC,'',ETL_T_TAB_ENG_NAME,3000001,@EXEC_MERGE_SQL,ETL_TX_DATE);
	IF @RTC <> 0 THEN SET OUT_RES_MSG = '12';LEAVE LABLE;END IF;
