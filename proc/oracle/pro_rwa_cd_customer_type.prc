CREATE OR REPLACE PROCEDURE RWA_DEV.PRO_RWA_CD_CUSTOMER_TYPE(p_data_dt_str IN  VARCHAR2, --数据日期 yyyyMMdd
...
BEGIN
    ...
    SELECT  REC.CLIENTID            AS CLIENTID        --参与主体ID
      ...
      AND (BCI.CUSTOMERTYPE='0310' OR BCI.CERTTYPE LIKE 'Ind%')           --零售客户
      WHERE REC.DATADATE=TO_DATE(p_data_dt_str,'YYYYMMDD')
      AND NOT EXISTS(SELECT 1 FROM RWA_DEV.RWA_CD_CUSTOMER_TYPE RCCT WHERE RCCT.CLIENTID=REC.CLIENTID AND RCCT.DATANO=p_data_dt_str)
      ;
     COMMIT;
    ...
    SELECT  REC.CLIENTID            AS CLIENTID        --参与主体ID
      ...
      AND (CCI.CUSTOMERTYPE='0310' OR CCI.CERTTYPE LIKE 'Ind%')           --零售客户
      WHERE REC.DATADATE=TO_DATE(p_data_dt_str,'YYYYMMDD')
      AND NOT EXISTS(SELECT 1 FROM RWA_DEV.RWA_CD_CUSTOMER_TYPE RCCT WHERE RCCT.CLIENTID=REC.CLIENTID AND RCCT.DATANO=p_data_dt_str)
      ;
   ...
   COMMIT;
    ...
    SELECT  REC.CLIENTID            AS CLIENTID        --参与主体ID
      ...
      FROM RWA_DEV.RWA_EI_CLIENT REC
      WHERE REC.DATADATE=TO_DATE(p_data_dt_str,'YYYYMMDD')
      AND (REC.REGISTSTATE='01' OR REC.CLIENTNAME LIKE '%招商银行股份有限公司香港分行%')   ---by chengang 20190910 修改      --“所在国家地区”为‘中国’ 中国(01) 非中国(02)
      AND (REC.CLIENTNAME LIKE '%银行%' OR REC.CLIENTNAME LIKE '%支行%' --OR REC.CLIENTNAME LIKE '%商行%'  经宋科确认删掉这个关键字
          ...
          /*  OR REC.CLIENTNAME LIKE '%花旗纽约%'*/ OR REC.CLIENTNAME LIKE '%中银香港%'--- by Chengang 20190910 去掉花旗纽约
          OR REC.CLIENTNAME LIKE '%工银亚洲%' OR REC.CLIENTNAME LIKE '%存放邮政储汇局%' OR REC.CLIENTNAME LIKE '%存放同业（乐山商行）%' )---by Chengang 20190910 add
      AND LENGTHB(CLIENTNAME)>9
      AND REC.CLIENTNAME NOT IN('加拿大蒙特利尔银行','重庆银行总行清算中心')---by Chengang 20190910  add
      AND NOT EXISTS(SELECT 1 FROM RWA_DEV.RWA_CD_CUSTOMER_TYPE RCCT WHERE RCCT.CLIENTID=REC.CLIENTID AND RCCT.DATANO=p_data_dt_str)
      ;
   ...
   /* INSERT INTO RWA_DEV.RWA_CD_CUSTOMER_TYPE(    --经黄征确认，这段删除
    ...
    SELECT  REC.CLIENTID            AS CLIENTID        --参与主体ID
      ...
      FROM RWA_DEV.RWA_EI_CLIENT REC
      WHERE REC.DATADATE=TO_DATE(p_data_dt_str,'YYYYMMDD')
     AND (REC.REGISTSTATE='02' OR  REC.CLIENTNAME LIKE '%花旗纽约%' OR REC.CLIENTNAME LIKE '%加拿大蒙特利尔银行%')  ---by chengang 20190910 修改             --“所在国家地区”为‘中国’ 中国(01) 非中国(02)
...
----------------------------------------------更新参与主体表的参与主体大小类----------------------------------------------------------
   ...
   UPDATE SET T.CLIENTTYPE = T2.RWACUSTYPE,T.CLIENTSUBTYPE=T2.RWACUSSUBTYPE;
    ...
    UPDATE RWA_DEV.RWA_EI_EXPOSURE T
		...
		COMMIT;
-----20191010 BY WZB 由于在NCM_CUSTOMER_INFO中有境外银行注册地为CHN，所以手动给注册地评级
    UPDATE RWA_EI_CLIENT SET RCERATING='0102',RCERAGENCY='01'
    WHERE CLIENTSUBTYPE='0206'
    AND (RCERATING is NULL
    OR RCERAGENCY IS NULL)
