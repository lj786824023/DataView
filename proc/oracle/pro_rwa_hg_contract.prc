CREATE OR REPLACE PROCEDURE RWA_DEV.PRO_RWA_HG_CONTRACT(
  ...
  /*
    存储过程名称:RWA_DEV.PRO_RWA_HG_CONTRACT
    实现功能:核心系统-回购-合同信息(从数据源核心系统将合同相关信息关联暴露信息全量导入RWA回购接口表合同信息表中)
    数据口径:全量
    跑批频率:月初运行
    版  本  :V1.0.0
    编写人  :LISY
    编写时间:2016-04-12
    单  位  :上海安硕信息技术股份有限公司
    源  表1 :RWA_DEV.RWA_HG_EXPOSURE|核心系统回购类信用风险暴露表
    目标表  :RWA_DEV.RWA_HG_CONTRACT|核心系统回购类合同信息表
    变更记录(修改人|修改时间|修改内容):
    */
  AS
  --创建一个自治事务
  ...
  BEGIN
    ...
    --Dbms_output.Put_line('RWA_DEV.RWA_HG_CONTRACT表当前插入的核心系统-回购数据记录为: ' || v_count || ' 条');
