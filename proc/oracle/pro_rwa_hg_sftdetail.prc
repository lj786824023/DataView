CREATE OR REPLACE PROCEDURE RWA_DEV.PRO_RWA_HG_SFTDETAIL(
			 											p_data_dt_str	IN	VARCHAR2,		--数据日期 yyyyMMdd
       											p_po_rtncode	OUT	VARCHAR2,		--返回编号 1 成功,0 失败
														p_po_rtnmsg		OUT	VARCHAR2		--返回描述
				)
  /*
    存储过程名称:RWA_DEV.PRO_RWA_HG_SFTDETAIL
  ...
  /*变量定义*/
  --定义存储过程名称并赋值
  v_pro_name VARCHAR2(200) := 'RWA_DEV.PRO_RWA_HG_SFTDETAIL';
  --定义异常变量
  v_raise EXCEPTION;
  --定义当前插入的记录数
  v_count INTEGER;
  ...
  BEGIN
    ...
    --对债券回购质押信息重复数据进行合并（暂时）
    EXECUTE IMMEDIATE 'TRUNCATE TABLE RWA_DEV.BRD_REPO_PORTFOLIO_TMP';
    insert into BRD_REPO_PORTFOLIO_TMP(datano,repo_reference,security_reference,belong_group)
    select datano,repo_reference,security_reference,belong_group
     from (select t.datano,
                  t.repo_reference,
                  t.security_reference,
                  t.belong_group,
                  row_number() over(partition by t.repo_reference, t.belong_group order by t.face_amount desc) as row_id
             from BRD_REPO_PORTFOLIO t WHERE T.DATANO=p_data_dt_str)
    where row_id = '1';
    commit;
    ...
    LEFT JOIN RWA_DEV.BRD_REPO_PORTFOLIO_TMP T2
    ON T1.ACCT_NO = T2.REPO_REFERENCE
    and t2.belong_group='2' --所属分组为'2资金系统'
    AND T1.DATANO = T2.DATANO
  LEFT JOIN RWA_DEV.BRD_BOND T3
    ON T2.SECURITY_REFERENCE = T3.BOND_ID
    AND T3.BELONG_GROUP='4' --所属分组为'2资金系统'
   ...
   WHERE T1.CASH_NOMINAL <> 0
    ...
    LEFT JOIN RWA_DEV.BRD_REPO_PORTFOLIO_TMP T2
    ON T1.ACCT_NO = T2.REPO_REFERENCE
    and t2.belong_group='2'
    AND T1.DATANO = T2.DATANO
  LEFT JOIN RWA_DEV.BRD_BOND T3
    ON T2.SECURITY_REFERENCE = T3.BOND_ID
    AND T3.BELONG_GROUP='4'
   AND T2.DATANO = T3.DATANO   
    WHERE T1.CASH_NOMINAL <> 0
         --AND T1.CLIENT_PROPRIETARY = 'T'  --买断式  源表为空，暂时更改
         AND T1.REPO_TYPE IN ( '4', 'RB')  --买入返售
