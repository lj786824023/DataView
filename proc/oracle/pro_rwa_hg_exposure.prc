CREATE OR REPLACE PROCEDURE RWA_DEV.PRO_RWA_HG_EXPOSURE(
			 											p_data_dt_str	IN	VARCHAR2,		--数据日期 yyyyMMdd
       											p_po_rtncode	OUT	VARCHAR2,		--返回编号 1 成功,0 失败
														p_po_rtnmsg		OUT	VARCHAR2		--返回描述
				)
  /*
    存储过程名称:RWA_DEV.PRO_RWA_HG_EXPOSURE
  ...
  /*变量定义*/
  --定义存储过程名称并赋值
  v_pro_name VARCHAR2(200) := 'RWA_DEV.PRO_RWA_HG_EXPOSURE';
  --定义异常变量
  v_raise EXCEPTION;
  --定义当前插入的记录数
  v_count INTEGER;
  ...
  BEGIN
    ...
    SELECT 
        ...
        T1.ORG_CD , --账务机构ID
        T2.ORGNAME  , --账务机构名称
        NVL(T3.INDUSTRYTYPE,'J6620') , --所属行业代码
        T4.ITEMNAME , --所属行业名称
        '0401'  , --业务条线
        ''  , --资产大类
        ''  , --资产小类
        ...
        --NVL(T1.CASH_NOMINAL, 0)+NVL(T1.ACCRUAL, 0),
    ...
    LEFT JOIN RWA_DEV.RWA_TEMP_PDLEVEL T5
    ...
    WHERE T1.CASH_NOMINAL<> 0 
      AND SUBSTR(T1.PRINCIPAL_GLNO, 1, 6) = '111103'   -----20190910 by wzb 过滤卖出回购
