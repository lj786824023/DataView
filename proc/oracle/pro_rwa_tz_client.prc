CREATE OR REPLACE PROCEDURE RWA_DEV.PRO_RWA_TZ_CLIENT(p_data_dt_str IN  VARCHAR2, --数据日期
                                              p_po_rtncode  OUT VARCHAR2, --返回编号
                                              p_po_rtnmsg   OUT VARCHAR2  --返回描述
)
  /*
    存储过程名称:RWA_DEV.PRO_RWA_TZ_CLIENT
  ...
  /*变量定义*/
  --定义存储过程名称并赋值
  v_pro_name VARCHAR2(200) := 'RWA_DEV.PRO_RWA_TZ_CLIENT';
  --定义异常变量
  v_raise EXCEPTION;
  --定义当前插入的记录数
  v_count INTEGER;
  ...
  BEGIN
    ...
    SELECT
                 TO_DATE(p_data_dt_str,'yyyyMMdd')                                      AS DataDate               --数据日期
                ,p_data_dt_str                                                          AS DataNo                 --数据流水号
                ,'XN_JRJG'                                                               AS ClientID               --参与主体ID            默认 中国中央政府
                ,'XN_JRJG'                                                               AS SourceClientID         --源参与主体ID
                ,'TZ'                                                                   AS SSysID                 --源系统ID
                ,'金融机构'                                                         AS ClientName             --参与主体名称          默认 中国中央政府
                 ,'9998'                                                             AS SOrgID                 --源机构ID
                ,'重庆银行股份有限公司'                                                             AS SOrgName               --源机构名称
                ,'1'                                                                   AS OrgSortNo              --所属机构排序号
                ,'9998'                                                             AS OrgID                  --所属机构ID
                ...
                ,'999999'                                                               AS IndustryID             --所属行业代码          默认 999999-未知
                ,'未知'                                                                 AS IndustryName           --所属行业名称          默认 999999-未知
                ,'02'                                                                   AS ClientType             --参与主体大类          默认 01-主权
                ,'0205'                                                                 AS ClientSubType          --参与主体小类          默认 0101-中国中央政府
                ,'01'                                                                   AS RegistState            --注册国家或地区        默认 01-境内
                ,'0104'                                                                 AS RCERating              --境外注册地外部评级
                ,'01'                                                                   AS RCERAgency             --境外注册地外部评级机构
                ,'JRJGZZJGDM'                                                         AS OrganizationCode       --组织机构代码
                ,'0'                                                                    AS ConsolidatedSCFlag     --是否并表子公司
                ,'0'                                                                    AS SLClientFlag           --专业贷款客户标识
                ,''                                                                     AS SLClientType           --专业贷款客户类型
                ,'020101'                                                               AS ExpoCategoryIRB        --内评法暴露类别        默认 020101-中央政府
    ...
    SELECT
                 TO_DATE(p_data_dt_str,'yyyyMMdd')                                      AS DataDate               --1数据日期
                ,p_data_dt_str                                                          AS DataNo                 --2数据流水号
                ,t.participant_code                                                     AS ClientID               --3参与主体ID            默认 中国中央政府
                ,t.participant_code                                                     AS SourceClientID         --4源参与主体ID
                ,'TZ'                                                                   AS SSysID                 --5源系统ID
                ,t.participant_name                                                     AS ClientName             --6参与主体名称          默认 中国中央政府
                 ,'9998'                                                             AS SOrgID                 --源机构ID
                ,'重庆银行股份有限公司'                                                             AS SOrgName               --源机构名称
                ,'1'                                                                   AS OrgSortNo              --所属机构排序号
                ,'9998'                                                             AS OrgID                  --所属机构ID
                ,'重庆银行股份有限公司'                                                             AS OrgName                --所属机构名称
                ,'999999'                                                               AS IndustryID             --12所属行业代码          默认 999999-未知
                ,'未知'                                                                 AS IndustryName           --13所属行业名称          默认 999999-未知
                ,''                                                                   AS ClientType             --14参与主体大类          默认 01-主权
                ,''                                                                 AS ClientSubType          --15参与主体小类          默认 0101-中国中央政府
                ,DECODE(t.geog_location,'002','02','01')                                AS RegistState            --16注册国家或地区        默认 01-境内
                ,'0104'                                                                 AS RCERating              --境外注册地外部评级
                ,'01'                                                                   AS RCERAgency             --境外注册地外部评级机构
                ,'JRJGZZJGDM'                                                           AS OrganizationCode       --组织机构代码
                ,'0'                                                                    AS ConsolidatedSCFlag     --是否并表子公司
                ...
                ,'0'                                                                    AS SSMBFlagSTD            --权重法标准小微企业标识
                ,NULL                                                                   AS AnnualSale             --公司客户年销售额
                ,DECODE(t.geog_location,'002','','CNH')                                 AS CountryCode            --注册国家代码
                ,''                                                                     AS MSMBFlag               --工信部微小企业标识
    FROM        FNS_BND_PARTICIPANT_B t
