CREATE OR REPLACE PROCEDURE RWA_DEV.PRO_RWA_ABS_ISSUE_ASSET_WSIB(
  ...
  /*
    存储过程名称:RWA_DEV.PRO_RWA_ABS_ISSUE_ASSET_WSIB
    实现功能:台账-资产证券化-发行机构-基础资产-补录铺底(从RWA资产证券化-发行机构-基础资产补录表中导入上期数据至RWA资产证券化-发行机构-基础资产补录铺底表中)
    数据口径:全量
    跑批频率:月初运行
    版  本  :V1.0.0
    编写人  :LISY
    编写时间:2016-12-08
    单  位  :上海安硕信息技术股份有限公司
    源  表1 :RWA.RWA_WS_ABS_ISSUE_UNDERASSET|资产证券化-发行机构-基础资产补录表
    目标表1 :RWA.RWA_WSIB_ABS_ISSUE_UNDERASSET|资产证券化-发行机构-基础资产铺底表
    变更记录(修改人|修改时间|修改内容):
    */
  AS
  --创建一个自治事务
  ...
  BEGIN
    ...
    SELECT
                ...
                ,T1.HTBH                        	 		 	 		 AS	HTBH                     --合同编号
                ,T1.HTYE                     	 		 		 			 AS	HTYE                     --合同余额

    FROM				RWA.RWA_WS_ABS_ISSUE_UNDERASSET T1           --资产证券化-发行机构-基础资产补录表，取最近一期补录数据铺底
    ...
    --Dbms_output.Put_line('RWA.RWA_WSIB_ABS_ISSUE_UNDERASSET表当前插入的台账-资产证券化-发行机构-基础资产铺底数据记录为: ' || v_count || ' 条');
    ...
    WHEN OTHERS THEN
       	 ...
       	 ROLLBACK;
   			 ...
   			 p_po_rtnmsg  := '资产证券化-发行机构-基础资产补录数据铺底('|| v_pro_name ||')处理失败！'|| sqlerrm||';错误行数为:'||dbms_utility.format_error_backtrace;
         RETURN;
